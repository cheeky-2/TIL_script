...
function weightCal(){
    ...
    // 계산식) 적정체중 = (입력키-100)*0.9
    // 1. 적정체중 구하는 변수제작 normalWeight
    const normalWeight = (userHeight.value-100)*0.9;
    // 2. 콘솔 로그로 적정체중 결과 출력
    console.log('적정체중:'+normalWeight);
    console.log(`당신은 적정체중은 ${normalWeight}kg 이며 ${userWeight.value-normalWeight}kg 초과되셨습니다.`)
    // input 요소일 경우 값 읽기/수정/삭제는 value 속성 사용
    // input이 아닌 다른 요소일경우 값 읽기/수정/삭제 textContent 또는 innerHTML 속성 사용
    result.innerHTML = `당신은 적정체중은 ${normalWeight}kg 이며<br> ${userWeight.value-normalWeight}kg <em>초과</em>되셨습니다.`
}