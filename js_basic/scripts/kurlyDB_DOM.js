...
let endIndex = starIndex+itemLoad; // 초기값+출력개수
console.log(itemLoad,starIndex,endIndex);
...
function loadMoreContent(){
    ...
    for(let i=starIndex; i<endIndex; i++){
        const newLi = document.createElement('li');//변수로 태그만들기
...
window.addEventListener('scroll',()=>{
    ...
    }else{ad[0].classList.remove('active');}
    // ad[1] 복습
    ...
    // 반복문 활용
    for(let i of ad){
        const adTop = i.getBoundingClientRect().top; // 뷰포트 탑 위치와의 거리 확인 변수
        console.log(i);
        if(adTop<window.innerHeight * 0.8){
