...
toBtn.addEventListener('click',()=>{
    if(toInput.value==''){//input의 값이 입력되지 않았을 경우
        ...
        li.innerHTML = `<span>${toInput.value}</span><button type="button" class="del_btn">X</button><hr>`
...
toInput.addEventListener('keydown',function(k){
    switch(k.key){
        case 'Enter':let li = document.createElement('li'); li.innerHTML = `<span>${toInput.value}</span><button type="button" class="del_btn">X</button>`; ul.appendChild(li); toInput.value = ''; const close = document.querySelectorAll('.del_btn'); for(let i of close){i.addEventListener('click',()=>{i.parentNode.remove();})}; const delContents = document.querySelectorAll('li span'); for(let i of delContents){i.addEventListener('click',()=>{i.style.textDecoration = 'line-through';})} break;
        default:'';}
    })
