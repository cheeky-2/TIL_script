# 오늘의 집 상품 주문영역 알고리즘
## 0. 페이지 경로 클릭 시 해당 페이지로 이동링크 o
* line 15~21 index.html : `<a href="#">반려동물</a>`
## 1. 작은 썸네일 이미지 이벤트 시 큰 이미지 변경 o
* thum4에 마우스를 올렸다 -> big이미지의 src속성값이 big4
## 2. 큰 이미지에서 마우스 드래그 시 이미지 좌우 변경 o
...
* color select 대상에 change 이벤트 적용하고 color 변경 시 호출하는 함수에 size select 활성화하는 함수 연결 - probuct.js line 1~12
...
## 18. 위 (12)번 진행한 상태에서 바로가기 클릭 시 비회원일 경우 로그인 새창 띄움, 회원일 경우 바로구매 페이지로 이동